...
func init() {
	...
	utilruntime.Must(greencostsv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(&prometheusURL, "prometheus-url", "",
		"URL of the Prometheus server used for network and ingress idle detection (e.g. http://prometheus:9090). Optional.")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "563838c3.greencosts.hstr.nl",
	...
	if err := controller.RegisterEnergyPriceSourceMetrics(ctrlmetrics.Registry, mgr.GetClient()); err != nil {
		setupLog.Error(err, "unable to register metrics", "collector", "EnergyPriceSource")
		os.Exit(1)
	}
	...
	if err := (&controller.EnergyPriceSourceReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "EnergyPriceSource")
		os.Exit(1)
	}
	if err := (&controller.EnergyAwareCronJobReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "EnergyAwareCronJob")
		os.Exit(1)
	}
	...
	if err := greencostswebhook.SetupHibernatePolicyWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "HibernatePolicy")
		os.Exit(1)
	}
	if err := greencostswebhook.SetupClusterHibernatePolicyWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "ClusterHibernatePolicy")
		os.Exit(1)
	}
