...
```bash
KUBECONFIG=/home/tscholten/.openclaw/local-k3s-root/kubeconfig.yaml \
...
```bash
make docker-build IMG=docker.io/tristanscholten/kube-greencosts-controller:vX.Y.Z
make deploy IMG=docker.io/tristanscholten/kube-greencosts-controller:vX.Y.Z
```
...
kubectl -n kube-greencosts-system set image \
  ...
  manager=docker.io/tristanscholten/kube-greencosts-controller:vPREVIOUS
kubectl -n kube-greencosts-system rollout status deploy/kube-greencosts-controller-manager
```
